...
class RapportSemestrielController extends Controller
{
    public function exportToExcel(Request $request)
    {
        ...
        $yearselec = $request->input('yearselec');
        $semestre = $request->input('semestre');

        // Vérifiez si les paramètres sont corrects
        if (!$yearselec || !$semestre) {
            return response()->json(['message' => "Les paramètres sont manquants  $yearselec ,$semestre"], 400);
        } else {
            if ($semestre == '1') {
                $startDate = date('Y-m-d', strtotime($yearselec . '-01-01'));
                $endDate = date('Y-m-d', strtotime($yearselec . '-06-30'));
            } else {
                $startDate = date('Y-m-d', strtotime($yearselec . '-07-01'));
                $endDate = date('Y-m-d', strtotime($yearselec . '-12-31'));
            }
            ...
            foreach ($candidats as $candidat) {
                ...
                // Assuming $candidat->odcuser_id is the column or a value to include
$speciality = Odcuser::select('detail_profession', 'id')
                     ->selectRaw('? AS odcuser_id', [$candidat->odcuser_id])
                     ->get();

// Decode the JSON data in detail_profession
$detail_profession = json_decode($speciality->detail_profession, true);

// Extract the 'speciality' value or default to an empty string if not found
$specialiteValue = $detail_profession['speciality'] ?? '';

// Optional: Output or use the $specialiteValue
// echo $specialiteValue;
            ...
            $fileName = "Rapport_du_Semestre_{$semestre}_{$yearselec}.Xlsx";
            $tempFile = tempnam(sys_get_temp_dir(), $fileName);
            $content = $writer->save($tempFile);
